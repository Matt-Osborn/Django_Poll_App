...
class QuestionModelTests(TestCase):
    def test_was_published_recently_with_future_question(self):
        ...
        is in the future.
        """
        ## set up a question with a pub_date in the future:
    ...
    def test_was_published_recently_with_old_question(self):
        """
        was_published_recently() returns False for questions whose pub_date
        is older then N days 
        (in this case 7 - this is hardcoded, and its unclear how to fix that).
        """
    ...
    def test_was_published_recently_with_recent_question(self):
        ...
        """
        time = timezone.now() - datetime.timedelta(days=6, hours=23, minutes=59, seconds=1)
        recent_question = Question(pub_date=time)
        self.assertIs(recent_question.was_published_recently(), True)


##Shortcut Function for Creating Test Questions:
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_past_question(self):
        """Questions with a pub_date from the past are displayed on the index page"""
        question = create_question(question_text="Past question.", days=-30)
        response = self.client.get(reverse("polls:index")) ##reverse() should really be called return or something 
        self.assertQuerysetEqual(
            response.context["latest_question_list"],
            [question],
        )
    def test_future_questions(self):
        """Questions with a pub_date from the future aren't displayed on the index page"""
